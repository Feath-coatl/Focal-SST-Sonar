...
class Config:
    FOREGROUND_CLASSES = [1, 2, 5, 6, 7, 8]
    WATER_CLASS = 4
    BG_CLASS = 3
    ...
    NOISE_MAX = 50
    NOISE_EPS = 0.8
    NOISE_MIN_SAMPLES = 25
    NOISE_MAX_POINTS = 100
...
class DataAugmentor:
    ...
    def extract_real_noise_templates(self, bg_data: np.ndarray) -> list:
        # [保留] 噪声提取逻辑保持不变，因为噪声可能离散且多发，DBSCAN 依然适用
...
def main():
    ...
    if args.mode in ['all', 'mix']:
        sample_size = int(total_files * 0.4) if args.mode == 'all' else total_files
        mix_files = random.sample(all_files, sample_size) if args.mode == 'all' else all_files
    ...
    if args.mode in ['all', 'cutoff']:
        print(f"\n[Mode: Cutoff] 正在执行 (100% 数据, 前缀 c_)...")
        count_saved = 0
        for fpath in tqdm(all_files, desc="Cutoff"):
            data = utils.read_txt(fpath)
            if data is None: continue
            ...
            if data_aug is not None:
                ...
                count_saved += 1
        print(f"  [Info] Cutoff 模式实际生成文件数: {count_saved} / {total_files}")
