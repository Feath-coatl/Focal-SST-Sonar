...
│   │   └── ...
│   ├── focalSST-master/        # [核心代码] 模型训练与推理框架 (基于 OpenPCDet/DSVT)
│   │   ├── pcdet/models/       # 模型定义 (Backbones, DenseHeads 等)
...
此外，原始数据的XYZ坐标系也需要特别说明：
- X轴指的是目标中心在声呐左/右方向的距离，正为右、负为左；
- Y轴指的是目标中心在声呐前/后方向的距离，正为前（远离声呐）；
...
- 由于声呐特殊的成像机制，它的Y轴分辨率要明显高于X和Z，并且不会随着距离增加而导致点间距变大。
- 声呐数据包含完整的3D信息，而不是激光雷达的2.5D数据，在声呐数据中可能出现两个目标处于同一（X,Y）但不同高度的情况。
