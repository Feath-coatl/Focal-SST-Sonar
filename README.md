...
- 声呐适配性:
  ...
  - **局部密度偏差**:修改Point-wise Attention。在声呐中，有效目标通常具有较高的局部点密度（高光区），而噪声（如混响散斑）是扩散的。网络将结合K-近邻密度对点特征进行显式加权，抑制离群噪声点。
...
- 优势与声呐适配:
  ...
  - **声纳阴影抑制**:在训练时，利用Ground Truth生成掩码。针对声呐特有的声学阴影(Acoustic Shadow)区域（通常位于物体后方），我们将 Focal Loss 的权重设为负样本并施加高惩罚，防止网络在阴影区错误地通过Dilation生成虚假特征。
